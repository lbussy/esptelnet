...
ESPTelnet::ESPTelnet()
{
	...
	welcomeMsg = strdup(ESPTELNET_WELCOME_MSG);
	rejectMsg = strdup(ESPTELNET_REJECT_MSG);
	minBlockSize = ESPTELNET_MIN_BLOCK_SIZE;
	collectingTime = ESPTELNET_COLLECTING_TIME;
	maxBlockSize = ESPTELNET_MAX_BLOCK_SIZE;
	pingTime = ESPTELNET_PING_TIME;
	pingRef = 0xFFFFFFFF;
	waitRef = 0xFFFFFFFF;
	telnetBuf = NULL;
	bufLen = 0;
	uint16_t size = ESPTELNET_BUFFER_LEN;
	...
	debugOutput = ESPTELNET_CAPTURE_OS_PRINT;
	if (debugOutput)
	{
		setDebugOutput(true);
	}
...
void ESPTelnet::setPort(uint16_t portToUse)
{
	port = portToUse;
	if (listening)
	{
...
void ESPTelnet::setWelcomeMsg(const char *msg)
{
	if (welcomeMsg)
	{
		free(welcomeMsg);
...
void ESPTelnet::setRejectMsg(const char *msg)
{
	if (rejectMsg)
	{
		free(rejectMsg);
...
void ESPTelnet::setMinBlockSize(uint16_t minSize)
{
	minBlockSize = min(max((uint16_t)1, minSize), maxBlockSize);
}

void ESPTelnet::setCollectingTime(uint16_t colTime)
{
	collectingTime = colTime;
}

void ESPTelnet::setMaxBlockSize(uint16_t maxSize)
{
	maxBlockSize = max(maxSize, minBlockSize);
}

bool ESPTelnet::setBufferSize(uint16_t newSize)
{
...
uint16_t ESPTelnet::getBufferSize()
{
	if (!telnetBuf)
	{
		return 0;
...
void ESPTelnet::setStoreOffline(bool store)
{
	storeOffline = store;
}

bool ESPTelnet::getStoreOffline()
{
	return storeOffline;
}

void ESPTelnet::setPingTime(uint16_t pngTime)
{
	pingTime = pngTime;
	if (pingTime == 0)
	{
...
void ESPTelnet::setSerial(HardwareSerial *usedSerial)
{
	usedSer = usedSerial;
}
...
void ESPTelnet::begin(unsigned long baud, SerialConfig config, SerialMode mode, uint8_t tx_pin)
{
	if (usedSer)
	{
...
void ESPTelnet::begin(unsigned long baud, uint32_t config, int8_t rxPin, int8_t txPin, bool invert)
{
	if (usedSer)
	{
...
void ESPTelnet::swap(uint8_t tx_pin)
{
	if (usedSer)
	{
		usedSer->swap(tx_pin);
...
void ESPTelnet::set_tx(uint8_t tx_pin)
{
	if (usedSer)
	{
...
void ESPTelnet::pins(uint8_t tx, uint8_t rx)
{
	if (usedSer)
	{
		usedSer->pins(tx, rx);
...
bool ESPTelnet::isTxEnabled(void)
{
	if (usedSer)
	{
...
bool ESPTelnet::isRxEnabled(void)
{
	if (usedSer)
	{
...
int ESPTelnet::availableForWrite(void)
{
	if (usedSer)
	{
...
void ESPTelnet::setDebugOutput(bool en)
{
	debugOutput = en;
	if (debugOutput)
	{
		actualObject = this;
#ifdef ESP8266
		os_install_putc1(ESPTelnet_putc); // Set system printing (os_printf) to ESPTelnet
	...
	else
	{
		if (actualObject == this)
		{
			...
			os_install_putc1(ESPTelnet_ignore_putc); // Ignore system printing
#else // ESP32
			// ToDo: How can be done this for ESP32 ?
#endif
			actualObject = NULL;
...
uint32_t ESPTelnet::baudRate(void)
{
	if (usedSer)
	{
...
void ESPTelnet::addTelnetBuf(char c)
{
	CRITCAL_SECTION_START
...
int ESPTelnet::telnetAvailable()
{
	int n = client.available();
	while (n > 0)
	{
		if (0xff == client.peek())
		{				   // If esc char for telnet NVT protocol data remove that telegram:
			client.read(); // Remove esc char
			n--;
			if (0xff == client.peek())
			{			  // If esc sequence for 0xFF data byte...
		...
		{			  // If next char is a normal data byte...
...
bool ESPTelnet::isClientConnected()
{
	return connected;
}

void ESPTelnet::setCallbackOnConnect(void (*callback)())
{
	callbackConnect = callback;
}

void ESPTelnet::setCallbackOnDisconnect(void (*callback)())
{
	callbackDisconnect = callback;
}

void ESPTelnet::handle()
{
	if (firstMainLoop)
	{
		firstMainLoop = false;
